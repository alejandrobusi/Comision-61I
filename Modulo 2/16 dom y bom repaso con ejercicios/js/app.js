...
const addTask = (x) => {
  ...
  printListToDocument(taskList);
};

const printListToDocument = (list) => {
  let listToDelete = document.getElementById('taskListElement');
  listToDelete.innerHTML = '';
  if (list.length !== 0) {
    list.map((task, i) => {
      const li = document.createElement('li');
      li.innerHTML = `<div class='d-flex my-2 justify-content-between'> ${task} <button class=' mx-3 btn btn-danger btn-sm' onclick=(deleteItemList(${i}))>Eliminar</button> </div>`;
